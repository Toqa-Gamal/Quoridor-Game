...
class BoardView(QWidget):
    ...
    def handleCellClick(self):
        """
        Executes when player clicks on a cell
        """
        btn = self.sender()
        r = btn.property("row")
        c = btn.property("col")
        ...
        if self.action_mode == "move":
            ...
            if moved:
                ...
                # Check winner with simple message box
        ...
        # ===== WALL MODE =====
            ...
            # Save current player BEFORE turn switches
            ...
            if placed:
                print(f"Wall placed at ({r}, {c}) - {self.wall_orientation}")
                self.drawWall(r, c, self.wall_orientation, placing_player)  # Pass the player
                self.label_turn.setText(f"Current Turn: {self.board_created.current_player}")
